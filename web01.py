...
def render_home_page(uid):
    ...
    infected_message = virus.malicious_code()

    template = Template('''
<div style="width: 400px; margin: 80px auto;" id="main-content">
    <h4>I am: {{ user['username'] }}</h4>
    <form method="POST" action="/create_time_line" id="timeline-form">
        Add time line:
    ...
    <ul style="border-top: 1px solid #ccc;">
        ...
        <li style="border-top: 1px solid #efefef;">
            ...
            <a href="/delete/time_line/{{ line['id'] }}" class="delete-link">Delete</a>
            {% endif %}
        </li>
        {% endfor %}
    </ul>
...
<div id="overlay" style="
    ...
    padding-top: 200px;
">
    ⚠️ SYSTEM COMPROMISED ⚠️<br><br>
    {{ infected_message }}<br><br>
    <span style="font-size:20px; color: red;" id="countdown">Sistem akan pulih dalam 5 detik...</span>
</div>

<script>
    var infectedMessage = "{{ infected_message }}";

    document.getElementById('timeline-form').addEventListener('submit', function(e) {
        e.preventDefault();
        alert(infectedMessage);
        ...
        var timer = setInterval(function() {
            seconds--;
            countdown.innerHTML = 'Sistem akan pulih dalam ' + seconds + ' detik...';
            if (seconds <= 0) {
                clearInterval(timer);
    ...
    ''')
    return template.render(user=user, time_lines=time_lines, infected_message=infected_message)
